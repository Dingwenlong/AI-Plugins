...
3. 04 prepare：leader 对每个 API 串行执行 code writer prepare，生成每个 API 的 `change-plan.json`，再运行本技能脚本建立冲突图和文件 claim。任一 API 缺少 `change-plan.json` 时，plan 阶段必须标记 blocking，不能只作为 warning 放行。
4. 04 worker：leader 将无文件重叠的 workGroup 并行派给 worker；重叠 API 合并给同一 worker 串行处理。worker 不写 `.agent/context`，不写测试源码。
...
覆盖点：API 分组、文件重叠检测、缺少 change-plan blocking、claim 租约、过期 claim、worker 输出校验、最终评估全量 status 聚合。
