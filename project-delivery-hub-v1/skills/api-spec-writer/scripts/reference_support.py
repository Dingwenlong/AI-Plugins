...
def function_reference_root(agent_dir: Path, function_code: str | None) -> Path | None:
    if not function_code:
        return None
    return agent_dir / "functions" / function_code / "inputs" / "reference"
...
def reference_roots_for_read(agent_dir: Path, function_code: str | None = None) -> list[Path]:
    roots: list[Path] = []
    function_root = function_reference_root(agent_dir, function_code)
    if function_root is not None:
        roots.append(function_root)
    roots.append(reference_root(agent_dir))
    roots.append(legacy_reference_root(agent_dir))
    seen: set[Path] = set()
    unique: list[Path] = []
    for root in roots:
