...
def build_ref_id(category: str, path: Path, metadata: dict[str, Any]) -> str:
    if category == "external_api":
        ...
        if api_codes:
            return f"external.{slugify(api_codes[0])}.{slugify(path.stem)}"
    if category == "db_schema":
        table_names = metadata.get("tableNames") or []
        if table_names:
            return f"db.{slugify(table_names[0])}.{slugify(path.stem)}"
    return f"{category.replace('_', '.')}.{slugify(path.stem)}"
