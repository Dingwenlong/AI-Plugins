...
- 专案时序图撰写标准，例如 `P240301_永豐商銀新大戶_时序图撰寫标准*.docx`。
...
- For project delivery diagrams, `DB` and `Redis` must be plain boxed participants (`participant "DB" as DB`, `participant "Redis" as Redis`), not PlantUML `database`/storage/cylinder icons. The final SVG/VSDX should show them with the same boxed participant head style as APP/Enterprise/IRIS.
