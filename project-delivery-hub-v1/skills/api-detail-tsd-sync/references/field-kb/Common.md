...
| 共用業務方法 | `CommonFunc.GetCommonCurrencyFunc`、`CommonFunc.EditCommonCurrencyFunc`、`CommonFunc.GetTransDebitCurrency` | 使用 `currencyCode` / `currencyName` / `currencyCodeArray` | DB 儲存結構若仍為 `CURRENAME`，只作來源/落庫說明。 |
