...
This standard defines the structural checks for DAWHO/New DAWHO API Detail worksheets. Visible style values are configuration-driven and must be read from `configs/api-detail-excel-style.json`.
...
- Report `校驗` as a wording issue even though it is not Simplified Chinese; it is a DAWHO delivery terminology preference.
