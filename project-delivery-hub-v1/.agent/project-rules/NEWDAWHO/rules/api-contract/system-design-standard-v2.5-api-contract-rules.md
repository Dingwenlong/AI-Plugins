...
Use this reference only when a DAWHO/New DAWHO task touches API contract reconciliation, TSD `API清單`, API Detail workbook content, naming, Redis/Appsetting/DB design evidence, or development-readiness decisions. Do not load it for simple path lookup or pure format checking.
...
- This reference belongs to design analysis. Do not extend its Redis/Appsetting/DB rules into `api-spec-writer-v2`, `api-code-writer-v2`, SQL fixture, or UT report skills unless the user explicitly asks.
