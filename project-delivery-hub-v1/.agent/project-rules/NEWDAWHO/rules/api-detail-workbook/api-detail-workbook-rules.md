...
3. 载入并遵循 `dawho-delivery-format-checker`。
...
- 写回 TSD `API清單`、API Detail `Api_List`、`涉及BackendAPI` / `後端來源` 或 API sheet 业务逻辑时，遇到历史账户取得、账户列表或转出账户类方法，一律收敛为 `GetEC0001`。
- 若目标位置是外部共用 API 或 TSD/API 清单调用口径，使用 `CommonUtil/GetEC0001`；若目标位置是内部实现、后端来源或业务逻辑口径，使用 `CommonFunc.GetEC0001`。
- 历史方法名 `ShowAcctList`、`GetAcctKind`、`GetAccountInfo`、`GetTransDebitAccount`、`GetBankAccountList`、`GetBankAccountListFunc`、`GetEC001AccountInfoFunc`、`GetAccountListByAPNOKindFunc` 不再作为正式接口来源写回，只能保留在历史证据、alias/migration notes 或问题清单中。
- 写回时必须删除 `TX_STATISTIC` 相关 DB 来源、BackendAPI、业务逻辑步骤、范例说明、字段来源与时序图交接内容；不得替换为其他表，也不得在正式 TSD/API Detail 中标为待确认后继续保留，除非用户另行提供新的权威来源。
- 如删除 `TX_STATISTIC` 后出现来源链缺口，在分析稿或问题清单说明缺口；正式 workbook/TSD 中只保留已确认来源或 `GetEC0001` 相关共用方法，不保留废弃表引用。
...
- 若调用 `dawho-delivery-format-checker` 的字体脚本，必须传入明确 `-Sheets` / 范围参数；不要使用会处理所有 API Detail worksheets 的默认命令。
