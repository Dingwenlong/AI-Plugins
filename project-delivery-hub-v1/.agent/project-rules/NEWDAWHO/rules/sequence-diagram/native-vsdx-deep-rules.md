...
description: 用于依专案冻版 PRD、TSD、API Detail、既有 VSDX/SVG/PlantUML 与本地时序图范例，产出或优化正式交付用的原生可编辑 Visio VSDX 时序图；兼容 DAWHO/New DAWHO 既有标准。PlantUML/SVG 只作为业务内容与视觉参考或明确降级输出。适用于使用者要求产出、优化、修正时序图、循序图、PlantUML、SVG、VSDX、Visio native shape、箭头、参与者、lifeline、alt/ref/opt/loop/group fragment、落版与 QA 规则时。兼容旧称：专案 PlantUML 时序图生成器。
---
...
父技能負責解析權威 PRD/TSD/API Detail 與 canonical API/欄位命名；本技能負責把凍版契約轉成正式交付用的 DAWHO 時序圖產物。
...
- 舊稱 `專案PlantUML時序圖產生器` 只作為觸發別名保留，不代表產線以 PlantUML/SVG 匯入為優先。
...
4. Local style examples, especially:
   ...
   - Built-in current DAWHO delivery-style baseline:
...
原生 VSDX template/master 必须携带 DAWHO 项目 Visio theme。有效 theme 模板需包含 `visio/theme/theme1.xml`，并且文件关系 type 以 `/officeDocument/2006/relationships/theme` 结尾。不要用裸 Office UML stencil 或缺少该 theme 的 native master 构建正式交付，因为 Visio UML shape 使用 `THEMEVAL()` 公式，缺少 theme 时会回退成默认黑/灰色，而不是 E.001 红/绿样式。

DAWHO 项目 theme 已内置在本技能的 `references/standard-examples/E001_native_reference.vsdx`。当调用方提供的 `-TemplateVsdx` 缺少项目 theme 时，native builder 必须自动改用内置 E.001 参考作为 fallback 模板。若调用方模板与内置 fallback 都缺少 theme，应让构建失败，不要产出默认 Office 配色的 VSDX。
...
一般 DAWHO 交付时，始终先创建或刷新 native reconstruction spec，再保留一份与该 spec 对齐的单一 `@startuml` 完整 PlantUML 文件，供文本/内容审阅。PlantUML 文件应只声明一次参与者，并用 `== ... ==` 分隔线连接所有情境章节。默认不要渲染 SVG 或 PNG 参考图。正式 VSDX 必须从 native reconstruction spec 构建，不能从 SVG import 构建。不要把多个独立 SVG 用嵌套 `<svg>` 区块拼接，因为这样会造成模块间视觉断裂与生命线断开。

DAWHO 交付场景中，VSDX 是默认交付物。用户通过本技能要求产出或最终化时序图时，除非用户明确只要 `.puml`、只要 `.svg`、只要文本，或本机环境无法运行 PlantUML/Visio，否则需创建下方 VSDX 输出。不要要求用户在另一个窗口再单独说一次“VSDX”。
...
正式 VSDX 必须通过 `scripts/build_native_visio_sequence.ps1`，从 `output/sequence_diagram/{functionCode}/{functionCode}_native_visio_spec.json` 产出，并使用 Visio UML masters 与原生 shape 模板库。若 native spec 缺失、过期，或 VSDX 构建命令没有使用该 spec，需先停止并修正；不得用 SVG-import VSDX 代替。DAWHO 系统设计标准 v2.5 要求交付 Visio source 且兼容 Visio 2013-2016；正式交接时需保留 native source VSDX，并在环境支持时保存/导出为 Visio 2013-2016 兼容格式。不要把新生成的功能图同步复制到 `v1.x Reference/` 作为标准交付步骤；官方生成输出应保留在 `output/sequence_diagram/{functionCode}/`。只有用户针对该次交付给出新的明确指示时，才额外创建 `v1.x Reference/` 副本，并记录为例外而非可复用规则。若因 PlantUML、Visio COM、必要 UML masters 或必要工具不可用而无法产出 VSDX，仍需产出 `.puml` 文本草稿，并明确报告缺少 native VSDX 是未完成交付项及具体阻塞原因。

在执行 VSDX 构建前，`{functionCode}_native_visio_spec.json` 的每个 page 都必须先补齐 `structureDecisions`。这是人工可读、机器可校验的结构预判记录，不取代 `frames` / `separators` / `messages`，但必须和它们一致。每条记录包含 `target`、`selected`、`reason`、`appliedTo`；`selected` 使用 `alt` / `opt` / `none` / `ref` / `group`。判断规则固定如下：有互斥正常分支或正常回传时用 `alt`，且必须有 frame condition 与至少一个 separator/else 分支；只有单一条件动作、没有 else 或正常分支内容时用 `opt`，且不得包含 separator；普通顺序处理、普通 request/response、自调用说明用 `none`；Common 引用仍用 `ref`；业务阶段仍用 `group`。出现 `查無...時`、`若...時`、`...失敗時` 等条件语义却没有相应 `structureDecisions`，或 `alt` 没有两个分支，或被 `appliedTo` 的 messages 没有落在 owning frame 内时，preflight 必须失败并停止生成。
...
- Conditional handling such as `查無...時`, `若...時`, or `...失敗時` must never be left as an ordinary self-call outside a fragment. Use native `alt` when the condition is mutually exclusive with a normal continuation/normal response, and place both the error/empty-result branch and the normal branch inside the same frame. Use native `opt` only when the conditional block has no explicit else/normal branch content to wrap.
- Native `alt` / `opt` frames must visibly enclose their owned branch content. A frame with only the header or condition label inside while the branch self-call, response, or normal return sits below the bottom border is not delivery-ready, even if Visio container membership exists.
- Use `ref over` for reusable internal Func/common references, not for hidden backend calls. The `ref` must name the implementation/common method being referenced, not the external API wrapper that called it. For example, if the main request is `ExchangeCommonUtil/GetCommonCurrency`, the `ref` should name `ExchangeCommonFunc.GetCommonCurrency`.
- Keep every synchronous call paired with a response arrow.
- For any participant-to-participant request that is an interface/API call, especially `APP -> Enterprise`, the visible arrow label must start with the full interface address from the frozen API Detail or canonical source, such as `ExchangeCommonUtil/GetCommonCurrency` or `Deposit/GetDemandDepositAccounts`, followed by a second line with the Traditional Chinese business description. Do not replace the interface address with a Chinese-only action such as `取常用幣別`, `取資料`, or `查詢資料`. This full-interface-address rule is for main-flow request arrows only; `ref` fragments show the internal Func/common target.
...
- For DAWHO delivery, declare `DB` and `Redis` as boxed participants only: use `participant "DB" as DB` and `participant "Redis" as Redis`. Never use PlantUML `database`, `collections`, `queue`, or other icon participant keywords for them unless the user explicitly asks for non-standard exploratory output.
...
- If a common method is referenced, put the internal Func/common method name, Chinese description, compact reference self-call, and SVG pointer inside the same `ref over Ent` block. Do not draw a separate Func/CommonFunc self-call arrow outside the `ref`, and do not omit the bottom orange reference-file strip when the self-call exists. In final SVG output, post-process the ref block to insert a compact red folded self-call arrow inside the ref block; the arrow must be anchored on the referenced participant lifeline (usually Enterprise), placed on the reserved spacer row below the internal method description, and use the same folded geometry as a normal self-call, only red. The `ref` frame background should be transparent, with only the SVG pointer lines highlighted orange; all `ref` text should be black.
- Leave two spacer lines between the internal Func/common method Chinese description and the SVG pointer text so the inserted inner self-call arrow has its own visual row and cannot be covered by the orange pointer block. For long Func names, split the method name across lines to avoid an over-wide `ref` box.
- Treat `CommonFunc` / `ExchangeCommonFunc` and similar internal Func classes as Enterprise internal implementation references or `ref over Ent`; do not draw them as participants.
- Treat `CommonUtil` / `ExchangeCommonUtil` as APIs requested through Enterprise only when APP actually calls that external API in the current scenario; keep those names on the main request arrow, not inside a Func `ref`.
- Visible common reference self-call notation is layer-sensitive: internal Func/common methods in `ref` use dot notation only (`CommonFunc.MethodName`, `ExchangeCommonFunc.MethodName`, for example `ExchangeCommonFunc.EditCommonCurrency\n更新常用幣別排序`). `CommonFunc/MethodName` and `ExchangeCommonFunc/MethodName` are invalid in `ref`. External API/wrapper names use slash notation (`CommonUtil/MethodName`, `ExchangeCommonUtil/MethodName`) only on real API request arrows. The orange pointer strip follows the referenced internal Func/common SVG basename, for example `04_CommonFunc.GenFntTranSeq 取得交易序號` or `77_ExchangeCommonFunc.GetCommonCurrency 取得常用幣別排序`.
...
创建或刷新图时，使用 `references/dawho-sequence-style.puml` 样式模板。
...
- `alt`/`else`/`group` branch separators should be DAWHO green `#1E5054`; in native VSDX, direct `else` operand separators use visible dashed lines carried by the native UML `Interaction operand` shape, while section divider double-lines remain solid. Keep the first `if` operand without an extra top separator, keep every following `else` operand separator visible, and bring section dividers plus native else separators to the top z-order so lifelines, frames, or messages cannot cover them. Do not add hand-drawn or overlay separator lines for `else`; each `else` head should have exactly one native operand separator. `opt` uses the Optional fragment's own `[參數]` slot and must not have `else` separators.
- Do not use PlantUML's old implicit black arrow / black arrow-text default for DAWHO delivery references. The SVG reference should use the current DAWHO message policy from `references/dawho-sequence-style.puml`, currently DAWHO red `#B01513` for message arrows and message labels unless a feature-specific rule explicitly says otherwise.
...
2. Put the internal Func/common method name, Chinese description, compact reference self-call, and SVG pointer inside the same `ref over Ent` block. Do not put the external caller API/wrapper path in the `ref`.
...
6. Use the exact reference wording:
   - Common SVG visible text: `{InternalFunc.MethodName}` + `\n{中文說明}\n　\n　\n{SVG basename without .svg}{中文說明}`. For wrapper/API calls, the main request arrow may show `CommonUtil/MethodName` or `ExchangeCommonUtil/MethodName`, but the `ref` text shows the internal Func/common target.
...
8. Every native `Message`, `Return Message`, and ordinary `Self Message` arrow must leave at least one connection-point interval from other visual shapes, including fragment frames, group/ref/alt headers, section divider/title boxes, notes, and orange pointer strips. One interval means the vertical distance between two adjacent default connector points on the participant lifeline, not a fixed inch value. Ordinary self-message folded arrows consume one connection-point interval and reserve one connection-point interval to neighboring rows; `ref`-only CommonFunc/CommonUtil self-calls keep the compact reference layout inside `ref`. Move the message row or the neighboring shape; do not allow arrows to touch, overlap, or visually merge with another shape.
...
- Every formal native spec must declare `messageStyle.policy`. Default to `e001-reference`, which follows `v1.x Reference/E.001_01.vsdx` / `references/standard-examples/E001_native_reference.vsdx` for native line/master behavior, while message / return / self-call label font color defaults to black `RGB(0,0,0)` unless an explicit local `textColor` override is required. Use `dawho-red` only when the user explicitly wants all message arrows/labels red. Use `preserve-native` only when the user explicitly wants raw Visio master colors. Do not leave message color policy implicit.
...
- Native `opt` fragments must use the real Visio UML `Optional fragment` master. Do not emulate opt by changing an `Alternative fragment` operator to `opt`, and do not draw a separate condition title/text box. Put the optional condition from `frames[].condition` into the Optional fragment's native `[參數]` placeholder, normalized as `[condition]`; if that placeholder is unavailable, stop the build and fix the shape library/template instead of falling back to an Alternative fragment.
...
- Message arrows must also clear non-message shapes by at least one connection-point interval. This applies to top/bottom edges of native fragment frames, group/ref/alt header tabs, section divider lines/title boxes, notes, and orange pointer strips. If a message row is visually adjacent to a frame/header/strip, move the arrow row down/up or move the frame so a full connection-point gap remains. Ordinary `Self Message` arrows consume one connection-point interval for the folded arrow and reserve one connection-point interval to neighboring message/frame rows; `ref`-only CommonFunc/CommonUtil self-calls keep the compact reference layout inside `ref`.
- Branch fit checks must use the real occupied range of each message, not only the message row's top coordinate. A native `Self Message` occupies the full folded-arrow span from `BeginY` to `EndY`; the if/else separator must be at least one connection-point interval below the folded arrow's lower end and at least one connection-point interval above the next relevant message or shape. If a self-call crosses or crowds a native `Interaction operand` separator, the output is wrong even when the message's nominal `top` value belongs to the intended branch.
...
- In native VSDX, internal Func/common text belongs on the native reference self-call arrow inside the `ref` frame; do not draw it as a main-flow Enterprise self-call, and do not leave a separate text box that collides with the arrow or orange pointer strip.
- In native VSDX, `ref` is a reusable-reference fragment, not a main-flow container. It may contain exactly the compact internal Func/common native reference self-call arrow that identifies the referenced method, plus the inset orange SVG pointer/description. Do not mix real request, response, return, APP display self-calls, external interface addresses, or Enterprise business-processing self-calls into a `ref`; place those flow messages outside the `ref` with fixed connector-point spacing. Conversely, if a main-flow message/self-call label begins with an internal Func name such as `CommonFunc.` or `ExchangeCommonFunc.`, the spec is wrong and must be rewritten as a `ref` fragment before VSDX delivery. Inside the `ref`, reject external wrapper paths such as `ExchangeCommonUtil/GetCommonCurrency` when the referenced implementation is `ExchangeCommonFunc.GetCommonCurrency`, and reject internal Func slash paths such as `ExchangeCommonFunc/EditCommonCurrency`; use the internal Func/common dot method name.
- In native VSDX, request/self-call/return arrows should use the corresponding Visio UML `Message`, `Self Message`, and `Return Message` masters. Color and label color are controlled by explicit native spec policy: `messageStyle.policy = "e001-reference"` follows the E.001 reference/native theme for arrow/master behavior, ignores generic `red=true` RGB forcing, and sets all message / return / self-call label fonts to black by default; `messageStyle.policy = "dawho-red"` applies DAWHO red `RGB(176,21,19)` to message line and label; `messageStyle.policy = "preserve-native"` keeps raw master defaults except where the native spec declares `textColor`. Use explicit per-message `lineColor` or `textColor` only when a local override is truly required.
...
- Supporting reference notes inside `ref` frames, such as `循序圖請參考：E.002 ...`, should use the same orange pointer-strip style as internal Func/common SVG references when the user asks for visual consistency. Increase the containing `ref` height and spacing so the orange strip, self message arrow, and arrow label never overlap.
...
- a native `ref` frame contains request/response/main-flow self-message arrows, or lacks the compact internal Func/common reference self-call when the ref is pointing to a common SVG;
...
8. Run the native layout planner against the spec before Visio COM rendering. The planner must express all vertical positions as participant connection-point rows, keep fragment/member content enclosed, set `ref` blocks as six-row atomic units, and leave at least one row between message arrows and other shapes; ordinary self messages consume one row for their folded span and reserve one row to the next message/frame row, while compact `ref` self-calls stay inside the six-row reference block.
...
- Final VSDX `ref` blocks use transparent/white frames with DAWHO green borders; fail validation if the entire ref block or header tab remains orange. Internal Func/common ref self-call arrows must be folded arrows anchored to the referenced participant lifeline and use the native message style policy from the spec. Any internal Func/common method drawn outside a native `ref` fragment is a validation failure. Only the `循序圖請參考：...` pointer background may be orange, and it must be inset inside the ref frame while fully covering the pointer text.
...
- The landing note lists actual VSDX output paths or a concrete blocker. Fail validation if it says VSDX is only a future optional step for a normal DAWHO delivery request. Do not list SVG/PNG output paths unless the user explicitly requested those extra artifacts.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File C:\Users\jimmy\plugins\newdawho-api-delivery-chain\skills\dawho-plantuml-sequence-writer\scripts\validate_native_visio_output.ps1 -VsdxPath path\to\file.vsdx
```
