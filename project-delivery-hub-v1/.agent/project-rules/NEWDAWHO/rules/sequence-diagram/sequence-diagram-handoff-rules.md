...
- Use `alt` for mutually exclusive branches, `opt` for optional work, `loop` for repeated calls, `group` for business stages, and `ref` only when the user or local standard allows cross-diagram references. `alt` must have at least two branches; when there is only one conditional block with no mutually exclusive continuation, use `opt`. Conditional handling such as `查無...時`, `若...時`, or `...失敗時` must never be a bare self-call: if a normal continuation or normal response exists as the other path, model both paths in one native `alt`; otherwise model the single conditional block as native `opt`. The visible `alt`/`opt` frame must fully wrap its branch self-calls, responses, and follow-up content, not only the header or condition label.
- Before native VSDX generation, record the same structure judgment in `{functionCode}_native_visio_spec.json` page-level `structureDecisions`: each decision states `target`, `selected` (`alt` / `opt` / `none` / `ref` / `group`), `reason`, and `appliedTo` indexes for the owning frame, separator, and messages. Conditional wording such as `查無...時`, `若...時`, or `...失敗時` must be covered by an `alt` or `opt` decision before handoff to the native builder.
...
- Use DAWHO message naming:
  ...
  - APP to Enterprise API: full interface address on the first line, such as `ExchangeCommonUtil/GetCommonCurrency` or `Deposit/GetDemandDepositAccounts`, plus a second line for the Traditional Chinese function description. Do not use only a Chinese action label such as `取常用幣別`. This naming belongs on the main-flow request arrow, not inside `ref`.
...
- Common SVG references should put the internal Func/common method name, Chinese description, and SVG pointer inside the same `ref` block. Do not draw a separate Func/CommonFunc self-call arrow outside the `ref`, and do not put the external caller API/wrapper path in the `ref`. Example: main-flow arrow `APP -> Ent: ExchangeCommonUtil/GetCommonCurrency\n取得常用幣別排序`; ref self-call/pointer `ExchangeCommonFunc.GetCommonCurrency\n取得常用幣別排序\n循序圖請參考：77_ExchangeCommonFunc.GetCommonCurrency 取得常用幣別排序`. For update-common-currency cases, write `ExchangeCommonFunc.EditCommonCurrency\n更新常用幣別排序`, not `ExchangeCommonFunc/EditCommonCurrency\n更新常用幣別排序`.
...
- Final SVG should post-process Common SVG `ref` blocks to insert a compact red folded self-call arrow inside the `ref` block, because PlantUML cannot natively draw a message arrow inside `ref`. The arrow must be anchored on the referenced participant lifeline, usually Enterprise, placed on the reserved spacer row below the internal Func/common description, and use the same folded self-call shape as normal message self-calls. The `循序圖請參考：...` pointer text must have an orange background strip that fully covers the pointer text, while the surrounding `ref` frame remains transparent.
- Common SVG `ref` blocks should reserve two spacer lines between the internal Func/common Chinese description and the SVG pointer text; long internal Func/common names may be split across lines to keep the ref width compact.
...
- Common SVG references must include `{InternalFunc.MethodName}\n{中文說明}\n循序圖請參考：{序號}_{InternalFunc.MethodName} {中文說明}` inside the same `ref` block. If the visible main request uses an external API path such as `CommonUtil/MethodName` or `ExchangeCommonUtil/MethodName`, the `ref` still names the internal Func/common method that API calls. Internal Func/common names in `ref` must use dot notation and must not use slash notation. Other supporting references use `循序圖請參考：{某某資料}`.
